...
class MHA(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        q = self.q_proj(x).unflatten(-1, (self.n_heads, -1)).transpose(-2, -3)  # (*, n_heads, L, head_dim)
...
class EncoderBlock(nn.Module):
    def __init__(
        self, d_model: int, n_heads: int, bias: bool = True, mlp_ratio: float = 4.0, dropout: float = 0.0
    ) -> None:
        super().__init__()
        self.sa_norm = nn.LayerNorm(d_model)
        self.sa = MHA(d_model, n_heads, bias, dropout)
...
class AudioTransformer(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(
            *[EncoderBlock(d_model, n_heads, bias, mlp_ratio, dropout) for _ in range(n_layers)]
        )
